...
class LogReader(object):
    ...
    def begin(self):
        ...
        if not os.path.isfile(self.logpath):
            # 日志文件不存在
            loger.debug("Log file not exists. Set pos to 0: %s", self.logpath)
            self.pos = 0
            return
    ...
    def read(self):
        ...
        if not os.path.isfile(self.logpath):
            # 日志文件还不存在
            loger.debug("Log file not exists yet. Return empty string: %s", self.logpath)
            return ""
    ...
    def read_last_lines(self, linenum):
        ...
        '''
        string = self.read()
        lines = string.splitlines(True)[-linenum:]   # 取最后20行
